...
from webstreaming import Person, Face
...
class ExtractEmbeddings:
    ...
    def process(self, db):
        ...
        for (i, imagePath) in tqdm(enumerate(imagePaths)):
            ...
            if len(detections) > 0:
                ...
                if confidence > CONFIDENCE:
                    ...
                    vec: np.array = self.embedder.forward()
        ...
        print(f"Known names: {list(set(knownNames))}")
        ...
        for (name, embedding) in zip(knownNames, knownEmbeddings):
            person_id = Person.query.filter_by(name=name).first().id
            emb = Face(person_id=person_id, embedding=embedding.tostring())
            db.session.add(emb)
